    ...
    // Initialize Express app and set up view engine
    const app = express();
    ...
    passport.deserializeUser(async (userId, done) => {
        try {
            const result = await db.query("SELECT * FROM user_info WHERE userid = $1", [userId]);
    ...
    app.get('/login', (req, res) => {
        req.isAuthenticated() ? res.redirect('/dashboard') : res.render('login.ejs', { error: null });
    });
    ...
    app.get('/dashboard/settings', (req, res) => {
        req.isAuthenticated() ? res.render('settings.ejs', { name: req.user.fullname, user: req.user.userid }) : res.status(401).redirect("/login");
    });
        ...
        try {
            ...
            req.logIn(user, (err) => {
                if (err) {
                    ...
                    return res.render("login", { error: "Login failed after registration" });
                }
